...
public class AlunoDAOImpl implements AlunoDAO {
    ...
    @Override
    public boolean cadastrarAluno(Connection conexao, Aluno aluno) {
        String sql = "INSERT INTO usuario_aluno (usuario_id, nome, sobrenome, cpf, sexo) VALUES (?, ?, ?, ?, ?)";

        try(PreparedStatement preparedStatement = conexao.prepareStatement(sql)) {
            preparedStatement.setLong(1, aluno.getId());
            preparedStatement.setString(2, aluno.getNome());
            preparedStatement.setString(3, aluno.getSobrenome());
            preparedStatement.setString(4, aluno.getCpf());
            preparedStatement.setString(5, aluno.getSexo().toString());

            int linhasAfetadas = preparedStatement.executeUpdate();
            return linhasAfetadas > 0;
        } catch (SQLException e) {
            Logger.getLogger(AlunoDAOImpl.class.getName()).log(Level.SEVERE, null, e);
            return false;
        }
    }

    @Override
    public boolean atualizarAluno(Connection conexao, Aluno aluno) {
        String sql = "UPDATE usuario_aluno SET nome=?, sobrenome=?, cpf=?, sexo=? WHERE usuario_id = ?";

        try (PreparedStatement preparedStatement = conexao.prepareStatement(sql)) {
            preparedStatement.setString(1, aluno.getNome());
            preparedStatement.setString(2, aluno.getSobrenome());
            preparedStatement.setString(3, aluno.getCpf());
            preparedStatement.setString(4, aluno.getSexo().toString());
            preparedStatement.setLong(5, aluno.getId());
    ...
    @Override
    public Aluno recuperarAlunoPeloId(Connection conexao, Long id) {
        String sql = "SELECT u.id, u.email, u.criado_em, u.atualizado_em, u.cargo, "
                ...
                + "FROM usuario u "
                + "INNER JOIN usuario_aluno a ON u.id = a.usuario_id "
        ...
        try (PreparedStatement preparedStatement = conexao.prepareStatement(sql)) {
            ...
            if(resultSet.next()) {
                ...
                return new Aluno(usuarioId, email, null, criadoEm, atualizadoEm, cargo, endereco, nome, sobrenome, cpf, sexo, null, null);
            }
        } catch (SQLException e) {
            Logger.getLogger(AlunoDAOImpl.class.getName()).log(Level.SEVERE, null, e);
            return null;
